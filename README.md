...
| **文件站** | 多级目录、列表/网格双视图、拖拽移动、回收站、批量上传、**macOS 原生风格终端** |
| **笔记** | Markdown 实时预览、Callout 容器、Mermaid 图表、任务列表、小说章节管理、图片粘贴上传、**自定义分类** |
| **采集** | 网页文本/图片/视频/音频批量采集、URL 序列展开、Jina.ai 深度提取 |
| **阅读器** | EPUB/TXT/Markdown 阅读、章节目录、进度记忆、字体切换 |
| **翻译** | 多语种互译、语法检查、历史记录 |
| **对话** | 4 家 AI 可切换、流式 SSE、多轮会话、工具调用、**交互小说角色（5 个男同向角色 + 长期记忆 + 剧情骰子）**、成人模式 |
| **壁纸** | 全局壁纸、轮播、文件中转站选图 |
...
- **人机验证**：滑块拼图验证码（sharp 切图 + 缺口对齐，误差 ≤5px）+ 独立登录页 login.html；通过后 30 天免验，VLC/播放器直连端点走 Basic Auth
