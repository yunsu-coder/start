    ...
    if (upRes.statusCode !== 200) {
      ...
      upRes.on('end', () => {
        try {
          ...
          let msg = e.error?.message || 'HTTP ' + upRes.statusCode;
          // 中转站常见错误：模型渠道缺失 → 给用户可操作的提示
          if (/No available channel/i.test(msg)) {
            msg += '（提示：模型名「' + (payloadObj.model || '?') + '」在该平台没有可用渠道，请到 导航栏 AK → 对话设置 里把模型改成平台支持的名称，如 grok-4.x / deepseek-v3 等）';
          }
          fail(new Error(msg));
        }
...
function authOK(req) {
  ...
  if (m) {
    ...
    if (a.length === b.length && c.length === d.length &&
        crypto.timingSafeEqual(a, b) && crypto.timingSafeEqual(c, d)) return true;
    return false;
  }
  // 会话 Cookie（登录页登录）
  const captcha = require('./captcha');
  return captcha.authCookieOK(req) === user;
}
