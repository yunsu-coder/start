...
async function scrapeBilibili(urlStr) {
  ...
  const os = require('os');
  const tmpDir = path.join(os.tmpdir(), 'yiwei_bili');
  ...
  for (let i = 0; i < durl.length; i++) {
    ...
    const size = await new Promise((resolve, reject) => {
      ...
      http.get(seg.url, {
        ...
        let written = 0;
        res.on('data', c => { written += c.length; file.write(c); });
        res.on('end', () => { file.end(); resolve(written); });
        res.on('error', e => { file.close(); reject(e); });
      }).on('error', reject);
    });
    results.push({ name, url: seg.url, size, tmpPath });
  }
