...
// 大文件（视频/音频）流式下载到磁盘，避免 OOM
function downloadToFile(urlStr, outPath, timeout = 120000, referer = null) {
  return new Promise((resolve, reject) => {
    const u = new URL(urlStr);
    const mod = u.protocol === 'https:' ? https : http;
    const ref = referer || `${u.protocol}//${u.host}/`;
    const req = mod.get(urlStr, {
      ...
      if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
        return downloadToFile(new URL(res.headers.location, urlStr).href, outPath, timeout, referer).then(resolve).catch(reject);
      }
      if (res.statusCode !== 200) return reject(new Error(`HTTP ${res.statusCode}`));
      const file = fs.createWriteStream(outPath);
      let size = 0;
      res.on('data', c => { size += c.length; file.write(c); });
      res.on('end', () => { file.end(); resolve(size); });
...
async function scrapeBilibili(urlStr) {
  ...
  if (urlStr.includes('b23.tv')) {
    try {
      var resolved = await new Promise((resolve) => {
        var req = https.get(urlStr, { timeout: 8000 }, res => {
          if ([301, 302, 307, 308].includes(res.statusCode)) resolve(res.headers.location);
          else resolve(null);
        });
  ...
  var bvidMatch = urlStr.match(/BV[a-zA-Z0-9]{10}/);
  if (!bvidMatch) return null;
  var bvid = bvidMatch[0];

  // 1. 获取视频信息
  var info = await biliGet('https://api.bilibili.com/x/web-interface/view?bvid=' + bvid);
  if (!info || info.code !== 0) throw new Error('B站API错误: ' + (info && info.message || '无响应'));
  var v = info.data;
  var cid = v.cid;
  var title = safeName(v.title.slice(0, 50));

  // 2. 获取播放地址
  var play = await biliGet('https://api.bilibili.com/x/player/playurl?bvid=' + bvid + '&cid=' + cid + '&qn=80&fnval=1&fourk=1');
  if (!play || play.code !== 0) throw new Error('播放地址获取失败: ' + (play && play.message || '无响应'));

  var durl = play.data && play.data.durl;
  if (!durl || !durl.length) throw new Error('无可用视频流');

  var os = require('os');
  var tmpDir = path.join(os.tmpdir(), 'yiwei_bili');
  if (!fs.existsSync(tmpDir)) fs.mkdirSync(tmpDir, { recursive: true });

  var results = [];
  for (var i = 0; i < durl.length; i++) {
    var seg = durl[i];
    var name = 'vid_' + title.slice(0, 30).replace(/_+/g, '_') + (durl.length > 1 ? '_p' + (i + 1) : '') + '.mp4';
    var tmpPath = path.join(tmpDir, name);
    // 使用 downloadToFile（支持重定向、Accept-Encoding: identity）
    var size = await downloadToFile(seg.url, tmpPath, 600000, 'https://www.bilibili.com/');
    results.push({ name: name, url: seg.url, size: size, tmpPath: tmpPath });
  }
  return { results: results, title: v.title };
}
...
async function doScrape(urls, type, opts = {}) {
  ...
  // URL 去重：只跳过之前采集成功的（有图片/文本且无错误）
  onProgress({ stage: 'dedup', total: urls.length });
  const existingSessions = listSessions();
  const alreadyScraped = urls.filter(url =>
    existingSessions.some(s => (s.url === url || (s.urls && s.urls.includes(url))) && s.errorCount === 0 && (s.imageCount > 0 || s.textCount > 0))
  );
  ...
  for (let idx = 0; idx < urls.length; idx++) {
    const url = urls[idx];
    onProgress({ stage: 'url_start', url, idx, total: urls.length });
    try {
      // ---------- B站视频专用抓取 ----------
      // ---------- B站视频专用抓取 ----------
        ...
        try {
          ...
          if (bili && bili.results) {
            for (const r of bili.results) {
              if (r.tmpPath && r.size > 1024) {
                const imgDir = path.join(sessionDir, 'images');
                fs.renameSync(r.tmpPath, path.join(imgDir, r.name));
                delete r.tmpPath;
                result.images.push({ name: r.name, url: r.url, size: r.size });
                result.imageCount++;
              }
            }
          ...
          const batchResults = await Promise.allSettled(
            batch.map(async (cand, bi) => {
              try {
                ...
                if (['.mp3', '.wav', '.ogg', '.flac', '.aac', '.m4a'].includes(ext.slice(0,5))) { const musName = 'mus_' + String(globalBase + i + bi + 1).padStart(4,'0') + ext.slice(0,5); const size = await downloadToFile(cand.orig, path.join(imgDir, musName), 300000, url); if (!size || size < 1024) return null; return { name: musName, url: cand.orig, size: size }; }
                if (['.mp4', '.webm', '.mov', '.mkv'].includes(ext.slice(0,5))) { const vidName = 'vid_' + String(globalBase + i + bi + 1).padStart(4,'0') + ext; const size = await downloadToFile(cand.orig, path.join(imgDir, vidName), 600000, url); if (!size || size < 1024) return null; return { name: vidName, url: cand.orig, size: size }; }
    ...
    onProgress({ stage: 'url_done', url, idx, total: urls.length, imageCount: result.imageCount, textCount: result.textCount, errorCount: result.errorCount });
  }

  onProgress({ stage: 'done', sessionId, imageCount: result.imageCount, textCount: result.textCount, errorCount: result.errorCount });
