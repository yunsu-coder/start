...
[FILES_DIR, TRASH_DIR, NOTES_DIR, WORKS_DIR].forEach(d => {
...
let _diskFreeCache = { val: '?', time: 0, ttl: 30000 };
function getDiskFree() {
  const now = Date.now();
  if (now - _diskFreeCache.time < _diskFreeCache.ttl) return _diskFreeCache.val;
  try {
    _diskFreeCache.val = execSync("df -h / | awk 'NR==2{print $4}'", { encoding: 'utf8', timeout: 3000 }).trim();
    _diskFreeCache.time = now;
  } catch {}
  return _diskFreeCache.val;
}
...
function getStatus() {
  ...
  return {
    ...
    cpu: os.loadavg()[0].toFixed(1), disk_free: getDiskFree(), uptime,
...
function listFiles(dirRel = '') {
  const dir = dirRel ? safeJoin(FILES_DIR, dirRel) : FILES_DIR;
...
function createFolder(dirRel) {
  const dir = safeJoin(FILES_DIR, dirRel || '');
  if (!dir) return { error: '非法路径' };
...
function renameFolder(dirRel, newName) {
  ...
  const oldPath = safeJoin(FILES_DIR, dirRel);
  if (!oldPath) return { error: '非法路径' };
  if (!fs.existsSync(oldPath)) return { error: '文件夹不存在' };
  const newPath = safeJoin(path.dirname(oldPath), newName);
  if (!newPath) return { error: '非法路径' };
...
function uploadFiles(parts, maxSize, subDir = '') {
  ...
  const targetDir = safeJoin(FILES_DIR, subDir || '');
  if (!targetDir) return { error: '非法路径' };
...
function restoreFromTrash(name) {
  const trashPath = safeJoin(TRASH_DIR, name);
  if (!trashPath) return { error: '非法路径' };
...
function deleteTrashItem(name) {
  const trashPath = safeJoin(TRASH_DIR, name);
  if (!trashPath) return { error: '非法路径' };
  if (!fs.existsSync(trashPath)) return { error: 'not found' };
  try {
...
function getFilePath(name) {
  // 安全拼接路径（支持子目录，拒绝穿越）
  const fp = safeJoin(FILES_DIR, name);
  if (fp && fs.existsSync(fp) && !fs.statSync(fp).isDirectory()) return fp;
  return null;
}
...
function listNotes() {
  return fs.readdirSync(NOTES_DIR).filter(f => f.endsWith('.json') && !f.startsWith('.')).map(f => {
    const raw = JSON.parse(fs.readFileSync(path.join(NOTES_DIR, f), 'utf8'));
    return {
      id: raw.id, title: raw.title, updated: raw.updated,
      preview: (raw.content || '').slice(0, 80),
      workId: raw.workId || '',
      chapterOrder: typeof raw.chapterOrder === 'number' ? raw.chapterOrder : 0,
...
function saveNote(body) {
  if (body.id && !safeJoin(NOTES_DIR, body.id + '.json')) return { error: '非法路径' };
  const id = body.id || crypto.randomBytes(8).toString('hex');
  const existing = body.id ? getNote(id) : null;
  const note = {
    id, title: body.title, content: body.content || '',
    workId: body.workId !== undefined ? body.workId : (existing?.workId || ''),
    chapterOrder: typeof body.chapterOrder === 'number' ? body.chapterOrder : (typeof existing?.chapterOrder === 'number' ? existing.chapterOrder : 0),
    category: body.category !== undefined ? (body.category || '') : (existing?.category || ''),
    created: body.created || existing?.created || new Date().toISOString(),
    updated: new Date().toISOString(),
  };
  // 更新前自动快照历史版本
  if (existing) snapshotNote(id);
  fs.writeFileSync(path.join(NOTES_DIR, id + '.json'), JSON.stringify(note, null, 2));
  return { id, updated: note.updated };
}

function getNote(id) {
  const fp = safeJoin(NOTES_DIR, id + '.json');
  if (!fp || !fs.existsSync(fp)) return null;
...
function deleteNote(id) {
  const fp = safeJoin(NOTES_DIR, id + '.json');
  if (!fp || !fs.existsSync(fp)) return { error: 'not found' };
  fs.unlinkSync(fp);
  // 清理作品中的章节引用
  fs.readdirSync(WORKS_DIR).filter(f => f.endsWith('.json')).forEach(f => {
    const wp = path.join(WORKS_DIR, f);
    const work = JSON.parse(fs.readFileSync(wp, 'utf8'));
    if (work.chapters?.includes(id)) {
      work.chapters = work.chapters.filter(c => c !== id);
      work.updated = new Date().toISOString();
      fs.writeFileSync(wp, JSON.stringify(work, null, 2));
    }
  });
  return { ok: true };
}
...
// ===== 笔记历史版本（保存前自动快照，保留最近 10 份）=====
const NOTES_VER_DIR = path.join(NOTES_DIR, '.versions');
const CATS_FILE = path.join(NOTES_DIR, '.categories.json');
const MAX_VERSIONS = 10;

function snapshotNote(id) {
  const fp = path.join(NOTES_DIR, id + '.json');
  if (!fs.existsSync(fp)) return;
  const verDir = path.join(NOTES_VER_DIR, id);
  fs.mkdirSync(verDir, { recursive: true });
  const ts = String(Date.now());
  fs.copyFileSync(fp, path.join(verDir, ts + '.json'));
  // 保留最近 MAX_VERSIONS 份
  const vers = fs.readdirSync(verDir).filter(f => f.endsWith('.json')).sort();
  while (vers.length > MAX_VERSIONS) fs.unlinkSync(path.join(verDir, vers.shift()));
}

function listNoteVersions(id) {
  const verDir = path.join(NOTES_VER_DIR, id);
  if (!fs.existsSync(verDir)) return [];
  return fs.readdirSync(verDir).filter(f => f.endsWith('.json'))
    .map(ts => {
      const stat = fs.statSync(path.join(verDir, ts));
      let title = '', updated = '';
      try {
        const note = JSON.parse(fs.readFileSync(path.join(verDir, ts), 'utf8'));
        title = note.title || ''; updated = note.updated || '';
      } catch {}
      return { ts: ts.replace(/\.json$/, ''), time: new Date(Number(ts.replace(/\.json$/, ''))).toISOString(), size: stat.size, title, updated };
    })
    .sort((a, b) => Number(b.ts) - Number(a.ts));
}

function getNoteVersion(id, ts) {
  const fp = safeJoin(NOTES_VER_DIR, id + '/' + ts + '.json');
  if (!fp || !fs.existsSync(fp)) return null;
  try { return JSON.parse(fs.readFileSync(fp, 'utf8')); } catch { return null; }
}

function restoreNoteVersion(id, ts) {
  const version = getNoteVersion(id, ts);
  if (!version) return { error: '版本不存在' };
  const fp = safeJoin(NOTES_DIR, id + '.json');
  if (!fp) return { error: '非法路径' };
  version.updated = new Date().toISOString();
  fs.writeFileSync(fp, JSON.stringify(version, null, 2));
  return { ok: true, note: version };
}
...
function listCategories() {
  try {
    const raw = JSON.parse(fs.readFileSync(CATS_FILE, 'utf8'));
    return Array.isArray(raw) ? raw : [];
  } catch { return []; }
}

function saveCategories(list) {
  fs.writeFileSync(CATS_FILE, JSON.stringify(list, null, 2));
}
...
function listWorks() {
  return fs.readdirSync(WORKS_DIR).filter(f => f.endsWith('.json')).map(f => {
    const raw = JSON.parse(fs.readFileSync(path.join(WORKS_DIR, f), 'utf8'));
    return { id: raw.id, title: raw.title, fandom: raw.fandom || '',
      chapterCount: (raw.chapters || []).length,
      updated: raw.updated };
  }).sort((a, b) => new Date(b.updated) - new Date(a.updated));
}

function saveWork(body) {
  if (body.id && !safeJoin(WORKS_DIR, body.id + '.json')) return { error: '非法路径' };
  const id = body.id || 'work_' + crypto.randomBytes(8).toString('hex');
  const work = {
    id, title: body.title, fandom: body.fandom || '', description: body.description || '',
    chapters: body.chapters || [],
    created: body.created || new Date().toISOString(),
    updated: new Date().toISOString(),
  };
  fs.writeFileSync(path.join(WORKS_DIR, id + '.json'), JSON.stringify(work, null, 2));
  return { id, updated: work.updated };
}
 ...
 */
function exportWork(workId, format = 'md') {
  const work = getWork(workId);
  if (!work) return null;

  const chapters = work.chapters
    .map(cid => {
      const note = getNote(cid);
